...
class Evaluator:
    ...
    def evaluate(self, predictions, references, documents, metrics=["rouge", "bertscore", "factkb"],
        ...
        # ported reference-free rewards (from summarize_rl; see reward_extras.py)
        if "triplet_coverage" in metrics:
            result_dict["triplet_coverage"] = self.calculate_triplet_coverage(predictions, triplets)
        if "hallu" in metrics:
    ...
    def calculate_bertscore(self, predictions, references):
        import evaluate
        bertscore = evaluate.load("../bertscore.py")
    ...
    # ---- ported reference-free reward metrics (see reward_extras.py) --------
    def calculate_triplet_coverage(self, predictions, triplets):
        """Mean per-entity content-token recall of each summary over its KB
        triplets. ``triplets[i]`` is the raw ``[head, relation, tail]`` list for
        prediction ``i`` (threaded from the dataset). Returns 0.0 when triplets
        are unavailable so an un-threaded call cannot silently inflate reward."""
        from reward_extras import triplet_coverage, to_triplets
        if not triplets:
            return 0.0
        res = []
        for i in range(len(predictions)):
            trip = to_triplets(triplets[i]) if i < len(triplets) else []
            res.append(triplet_coverage(predictions[i], trip))
        return float(np.mean(res)) if res else 0.0

    def calculate_ungrounded_fact_penalty(self, predictions, documents):
        """Mean fraction of each summary's checkable facts (units/quantities)
        that are NOT grounded in the source document. Higher = worse."""
        from reward_extras import ungrounded_fact_penalty
...
def configure_model_loading(args):
    ...
    elif args.loading_mode == "bf16":
        # 从预训练模型加载配置
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(args.model_name_or_path)
        config.alpha_et_hidden_size = args.alpha_et_hidden_size
        config.dropout_rate = args.dropout_rate
        config.sqrt_dimension = args.sqrt_dimension
        config.use_fc_fp32 = args.my_fc_fp32
        config.sqrt_method = args.sqrt_method
        config.sqrt_dimension_first = args.sqrt_dimension_first
        config.sqrt_dimension_second = args.sqrt_dimension_second
...
def configure_model_loading_sft(args):
    ...
    if args.loading_mode == "nf4":
        ...
        if 'Llama' in args.model_name_or_path:
            from transformers import LlamaForCausalLM_sft
            model = LlamaForCausalLM_sft.from_pretrained(
                args.model_name_or_path,
                config=config,
                device_map="balanced"
            )
        elif 'opt' in args.model_name_or_path:
            from transformers import OPTForCausalLM_sft
            model = OPTForCausalLM_sft.from_pretrained(
                args.model_name_or_path,
                config=config,
                device_map="balanced"
            )
        
        elif 'gpt-neo' in args.model_name_or_path:
            from transformers import GPTNeoForCausalLM_sft
            model = GPTNeoForCausalLM_sft.from_pretrained(
                args.model_name_or_path,
                config=config,
                device_map="balanced"
            )
            ...
            from transformers import MistralForCausalLM_sft
            model = MistralForCausalLM_sft.from_pretrained(
                args.model_name_or_path,
                config=config,
                device_map="balanced"
            )
...
def load_dataset_summary(dataset, data_type):
    input_dir = f"../data/{dataset}/"
    train, validation, test = [], [], []
    with open(os.path.join(input_dir, "train_{}.jsonl".format(data_type)), "r", newline='\n') as fin:
        json_list = list(fin)
        for i, row in enumerate(json_list):
            row = json.loads(row)
            train.append([row["document"], row["summary"], row["presumm"]])
        print('training set len ', len(train))
    with open(os.path.join(input_dir, "test_{}.jsonl".format(data_type)), "r") as fin:
        json_list = list(fin)
        for i, row in enumerate(json_list):
            row = json.loads(row)
            validation.append([row["document"], row["summary"], row["presumm"]])
    with open(os.path.join(input_dir, "test_{}.jsonl".format(data_type)), "r") as fin:
        json_list = list(fin)
        for i, row in enumerate(json_list):
            row = json.loads(row)
...
def load_dataset_summary_new(dataset, data_type):
    if dataset == 'cnn_dailymail_new':
        head_name = 'cnndm'
    elif dataset == 'xsum':
        head_name = 'xsum'
    ...
    input_dir = f"../data/{head_name}/"

    train, validation, test = [], [], []
    with open(os.path.join(input_dir, "{}_train_{}.jsonl".format(head_name, data_type)), "r", newline='\n') as fin:
        json_list = list(fin)
        for i, row in enumerate(json_list):
            row = json.loads(row)
            train.append([row["document"], row["summary"], row["presumm"]])
        print('training set len ', len(train))
    with open(os.path.join(input_dir, "{}_test_{}.jsonl".format(head_name, data_type)), "r") as fin:
        json_list = list(fin)
        for i, row in enumerate(json_list):
            row = json.loads(row)
            validation.append([row["document"], row["summary"], row["presumm"]])
...
def load_dataset_ours(data_type="default"):
    """Load the summarize_rl Korean corpus (this repo's data/).

    Delegates to reward_extras.load_ours_dataset (pure-stdlib, unit-testable).
    Rows are [document, summary, presumm(keyfacts joined), triplets]; train/eval
    split by the `split` field; test_scenarios_ko.jsonl is the test set.
    """
    from reward_extras import load_ours_dataset
    train, validation, test = load_ours_dataset()
    print('training set len ', len(train))
    print('test set len ', len(test))
    return train, validation, test
...
def load_dataset(dataset, data_type):
    if dataset in ["dbpedia_processed", "pubmedqa_processed"]:
        ...
        return load_dataset_summary(dataset, data_type)
    elif dataset in ["cnn_dailymail_new", "xsum", "wikihow", "nyt50"]:
        return load_dataset_summary_new(dataset, data_type)
    elif dataset in ["summarize_rl_ko"]:
        return load_dataset_ours(data_type)


def template_input_decoder(row, dataset):
    if dataset == "summarize_rl_ko":
        return f"[보고서]\n{row[0]}\n\n위 보고서를 군사 표준용어를 사용하여 간결히 요약하시오.\n\n[요약]\n"
    if dataset == "wikihow":
        return f"Article: {row[0]}. Summary of the above article:"
    if dataset == "xsum":
        return f"News article: {row[0]}. Summary of the above news article in one sentence:" 
    if dataset == "multi_news":
        return f"News article: {row[0]}. Summary of the above news article:"
    if dataset == "cnn_dailymail" or dataset == "cnn_dailymail_new" or dataset == "nyt50":
        return f"News article: {row[0]}. Summary of the above news article:"
...
def presumm_input_decoder(row,dataset):
    if dataset == "summarize_rl_ko":
        return f"[보고서]\n{row[2]}\n\n위 보고서를 군사 표준용어를 사용하여 간결히 요약하시오.\n\n[요약]\n"
    if dataset == "wikihow":
        return f"Article: {row[2]}. Summary of the above article:"
    if dataset == "xsum":
        return f"News article: {row[2]}. Summary of the above news article in one sentence:"
    if dataset == "cnn_dailymail" or dataset == "cnn_dailymail_new" or dataset == "nyt50":
...
def get_null_input_decoder(row, dataset):
    if dataset == "summarize_rl_ko":
        return f"[보고서]\n\n위 보고서를 군사 표준용어를 사용하여 간결히 요약하시오.\n\n[요약]\n"
    if dataset == "wikihow":
        return f"Article: . Summary of the above article:"
    if dataset == "xsum":
        return f"News article: . Summary of the above news article in one sentence:"
    if dataset == "multi_news":
...
def template_input_encoder_decoder(row, dataset):
    ...
    if dataset == "multi_news":
        return f"Summarize this following article in one or two sentences: {row[0]}"
    if dataset == "cnn_dailymail" or dataset == "cnn_dailymail_new":
